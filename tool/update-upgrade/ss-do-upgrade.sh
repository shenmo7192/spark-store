...
echo "UOS中系统依赖无法使用第三方下载工具，使用apt-fast下载本体"
  ...
  do
    ...
    if sudo bwrap --dev-bind / / --bind '/opt/durapps/spark-store/bin/apt-fast-conf/sources.list.d/sparkstore.list' /etc/apt/sources.list.d/sparkstore.list apt install "$PKG_NAME" -y -o Dir::Cache="/etc/aptss/"  >/dev/null 2>&1;
    then
      echo "$PKG_NAME 已更新"
    else
